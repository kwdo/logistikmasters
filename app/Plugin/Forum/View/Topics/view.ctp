					...
					<td valign="top" style="width: 25%">
						...
						<?php if ($settings['enable_gravatar']) { ?>
							...
							<?php
								...
								if (isset($post['User']['UserProfile']['picupload'])) {
									$imageFile = $post['User']['UserProfile']['picupload'];
								}
								if(isset($post['User']['UserProfile']['picupload_dir'])) {
									$imagePath = $post['User']['UserProfile']['picupload_dir'];
								}

								$customImage = false;
								if ($imageFile && $imagePath) {
									$customImage = true;
									echo "<img src=\"/files/user_profile/picupload/{$imagePath}/thumb_{$imageFile}\" alt=\"Profilbild\" />";
								}

								if (!$customImage) {
									if ($settings['enable_gravatar']) {
										echo $this->Gravatar->image($post['User'][$config['userMap']['email']]);
									}
								}
							?>
							</div>
