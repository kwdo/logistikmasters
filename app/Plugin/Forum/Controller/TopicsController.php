	...
	public function add($slug, $type = '') {
		...
		if (!empty($this->request->data)) {
			...
			if ($topic_id = $this->Topic->add($this->request->data['Topic'])) {
                ...
                $newTopic = $this->Topic->read();

				$this->Profile->increaseTopics($user_id);

                $body = "Es wurde ein neues Thema erstellt:" . PHP_EOL;
                $body .= 'https://' . $_SERVER['HTTP_HOST'] . '/forum/topics/view/' . $newTopic['Topic']['slug'] . PHP_EOL . PHP_EOL;
                $body .= $newTopic['Topic']['title']  . PHP_EOL . $this->request->data['Topic']['content'] . PHP_EOL;
                $recipents = array(
                    'michael.knoch@knochwerke.de',
                    'logistikmasters@springer.com',
                    'teresa.hoeller@springer.com',
					'michael.cordes@springer.com',
					'andre.giesse@springer.com'
                );
                foreach($recipents as $recipent) {
                    mail($recipent, 'Neuer Eintrag im LOGISTIK MASTERS-Forum', $body, 'From: LOGISTIK MASTERS-Forum <logistikmasters@springer.com>');
                }
