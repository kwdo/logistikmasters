 ...
 */
	public static function niceShort($dateString = null, $timezone = null) {
		...
		if (self::isToday($dateString, $timezone)) {
			$ret = __d('cake', 'Heute, %s', self::_strftime("%H:%M", $date));
 ...
 */
	public static function timeAgoInWords($dateTime, $options = array()) {
		...
		if ($diff > abs($now - self::fromString($end))) {
			...
			$relativeDate .= $f >= 1 && $years > 0 ? ($relativeDate ? ', ' : '') . __dn('cake', '%d Jahr', '%d Jahre', $years, $years) : '';
			$relativeDate .= $f >= 2 && $months > 0 ? ($relativeDate ? ', ' : '') . __dn('cake', '%d Monat', '%d Monate', $months, $months) : '';
			$relativeDate .= $f >= 3 && $weeks > 0 ? ($relativeDate ? ', ' : '') . __dn('cake', '%d Woche', '%d Wochen', $weeks, $weeks) : '';
			$relativeDate .= $f >= 4 && $days > 0 ? ($relativeDate ? ', ' : '') . __dn('cake', '%d Tag', '%d Tage', $days, $days) : '';
			$relativeDate .= $f >= 5 && $hours > 0 ? ($relativeDate ? ', ' : '') . __dn('cake', '%d Stunde', '%d Stunden', $hours, $hours) : '';
			$relativeDate .= $f >= 6 && $minutes > 0 ? ($relativeDate ? ', ' : '') . __dn('cake', '%d Minute', '%d Minuten', $minutes, $minutes) : '';
			$relativeDate .= $f >= 7 && $seconds > 0 ? ($relativeDate ? ', ' : '') . __dn('cake', '%d Sekunde', '%d Sekunden', $seconds, $seconds) : '';

			if (!$backwards) {
				$relativeDate = __d('cake', 'vor %s', $relativeDate);
			}
		}
